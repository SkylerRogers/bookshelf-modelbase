  ...
  var model = bookshelf.Model.extend({
    ...
    validateSave: function (model, attrs, options) {
      ...
      if ((model && !model.isNew()) || (options && (options.method === 'update' || options.patch === true))) {
        ...
        validation = Joi.validate(attrs, this.validate.optionalKeys(optionalKeys), { allowUnknown: true })
      } else {
        validation = Joi.validate(this.attributes, this.validate, { allowUnknown: true })
      }
